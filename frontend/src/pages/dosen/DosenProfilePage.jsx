...
import { formatStatusLabel, statusVariant } from "./dosenHelpers.js";
...
const DosenProfilePage = () => {
  ...
  return (
    <DashboardLayout title="Profil Dosen">
      <section className="page-header">
        <div>
          <span className="eyebrow">PROFIL</span>
          <h2>Profil Dosen</h2>
          <p>Informasi akun dosen.</p>
        ...
        <Card title="Informasi Akun">
          {loading ? (
            ...
            <div className="detail-grid">
              ...
              <span><b>Role</b><Badge variant="success">{formatStatusLabel(user.role)}</Badge></span>
              <span><b>Nomor Induk</b>{user.nomor_induk || "-"}</span>
              <span><b>No HP</b>{user.no_hp || "-"}</span>
              <span><b>Status</b><Badge variant={statusVariant(user.status)}>{formatStatusLabel(user.status)}</Badge></span>
            </div>
          )}
        </Card>
      ) : null}
...
export default DosenProfilePage;
