...
const getEquipment = (item) => item?.equipments || item?.equipment || null;
const getLab = (item) => item?.laboratories || item?.laboratory || item?.lab || getEquipment(item)?.laboratories || null;
const getUser = (item) => item?.users || item?.user || item?.penanggung_jawab || null;
...
const getEquipmentLabel = (equipment) => {
  if (!equipment) return "-";
  const code = getEquipmentCode(equipment);
  return `${code} - ${equipment.nama_alat || "-"}`;
};
...
const AdminMaintenancesPage = () => {
  ...
  const loadData = async () => {
    ...
    setError("");
    try {
      const [maintenanceData, equipmentData] = await Promise.all([get("/maintenances"), get("/equipments")]);
  ...
  const fillForm = (data) => {
    setForm({
      equipment_id: data.equipment_id || getEquipment(data)?.id || "",
      jenis_perawatan: data.jenis_perawatan || "rutin",
      tanggal_perawatan: toDateInput(data.tanggal_perawatan),
      teknisi: getUser(data)?.name || "",
      biaya: data.biaya ?? "",
      status: data.status || "proses",
      status_akhir_alat: getEquipment(data)?.status || "tersedia",
  ...
  const validateForm = () => {
    ...
    if (form.biaya !== "" && (!Number.isFinite(Number(form.biaya)) || Number(form.biaya) < 0)) {
      return "Biaya harus berupa angka dan tidak boleh negatif.";
    }
    if (form.status === "selesai" && !form.status_akhir_alat) {
      return "Status akhir alat wajib dipilih saat perawatan selesai.";
    }
    return "";
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (modalMode === "edit" && selectedMaintenance) {
        await put(`/maintenances/${selectedMaintenance.id}`, payload);
        setNotice("Perawatan berhasil diperbarui.");
      } else {
        await post("/maintenances", payload);
  ...
  const columns = [
    ...
    { key: "kode_perawatan", label: "Kode Perawatan", render: (row) => getMaintenanceCode(row) },
    { key: "alat", label: "Alat", render: (row) => getEquipment(row)?.nama_alat || "-" },
    { key: "laboratorium", label: "Laboratorium", render: (row) => getLab(row)?.nama_lab || "-" },
    { key: "tanggal_perawatan", label: "Tanggal Perawatan", render: (row) => formatDate(row.tanggal_perawatan) },
    { key: "jenis_perawatan", label: "Jenis Perawatan", render: (row) => formatLabel(row.jenis_perawatan) },
    { key: "teknisi", label: "Teknisi", render: (row) => getUser(row)?.name || "-" },
    {
      key: "status",
      label: "Status",
  ...
  return (
    <DashboardLayout title="Manajemen Perawatan">
      <section className="page-header">
        <div>
          <span className="eyebrow">PERAWATAN</span>
          <h2>Manajemen Perawatan</h2>
          <p>Kelola jadwal, progres, dan hasil perawatan alat laboratorium.</p>
        </div>
      ...
      <Card>
        <div className="toolbar">
          <input
            ...
            placeholder="Cari kode, alat, laboratorium, teknisi, atau tindakan..."
          />
        ...
        <DataTable
          ...
          loading={loading}
          emptyTitle={maintenances.length ? "Belum ada data" : "Belum ada data perawatan"}
          emptyMessage={
            maintenances.length
              ? "Tidak ada perawatan yang sesuai filter."
              : "Data perawatan akan muncul setelah admin menambahkan jadwal atau riwayat perawatan alat."
          }
          actions={(row) => (
      ...
      <Modal
        ...
        title={modalMode === "edit" ? "Edit Perawatan" : "Tambah Perawatan"}
        onClose={closeModal}
      >
        ...
        <form className="form-grid" onSubmit={handleSubmit}>
          <label>
            ...
            <select value={form.equipment_id} onChange={(event) => updateForm("equipment_id", event.target.value)}>
              ...
              {equipments.map((equipment) => (
                <option value={equipment.id} key={equipment.id}>{getEquipmentLabel(equipment)}</option>
              ))}
            </select>
          </label>
          <label>
            ...
            <select value={form.jenis_perawatan} onChange={(event) => updateForm("jenis_perawatan", event.target.value)}>
              {jenisOptions.map((jenis) => (
                <option value={jenis} key={jenis}>{formatLabel(jenis)}</option>
              ))}
            </select>
          </label>
          <label>
            Tanggal Perawatan
            <input type="date" value={form.tanggal_perawatan} onChange={(event) => updateForm("tanggal_perawatan", event.target.value)} />
          </label>
          <label>
            Teknisi
            <input value={form.teknisi} onChange={(event) => updateForm("teknisi", event.target.value)} placeholder="Opsional" />
          </label>
          <label>
            Biaya
            <input type="number" min="0" value={form.biaya} onChange={(event) => updateForm("biaya", event.target.value)} />
          </label>
          <label>
            Status
          ...
          <label className="form-span">
            Deskripsi Masalah
            <textarea value={form.deskripsi_masalah} onChange={(event) => updateForm("deskripsi_masalah", event.target.value)} />
          </label>
          <label className="form-span">
            Tindakan
            <textarea value={form.tindakan} onChange={(event) => updateForm("tindakan", event.target.value)} />
          </label>
          <label className="form-span">
            Catatan
            <textarea value={form.catatan} onChange={(event) => updateForm("catatan", event.target.value)} />
          </label>
      ...
      <Modal open={modalMode === "detail"} title="Detail Perawatan" onClose={closeModal}>
        {selectedMaintenance ? (
          <div className="detail-grid">
            ...
            <span><b>Alat</b>{getEquipmentLabel(getEquipment(selectedMaintenance))}</span>
            <span><b>Laboratorium</b>{getLab(selectedMaintenance)?.nama_lab || "-"}</span>
            ...
            <span><b>Teknisi</b>{getUser(selectedMaintenance)?.name || "-"}</span>
      ...
      <Modal
        ...
        title="Tandai Perawatan Gagal"
        onClose={() => (!submitting ? setConfirmTarget(null) : null)}
      >
        {error && confirmTarget ? <div className="alert danger">{error}</div> : null}
        <div className="confirm-content">
          <p>Perawatan ini akan ditandai dengan status gagal tanpa menghapus histori.</p>
          <strong>{getMaintenanceCode(confirmTarget)} - {getEquipment(confirmTarget)?.nama_alat || "-"}</strong>
        </div>
        <div className="modal-footer">
          ...
          <button type="button" className="button danger" onClick={handleDelete} disabled={submitting}>
            {submitting ? "Memproses..." : "Ya, Tandai Gagal"}
          </button>
        </div>
      </Modal>
    </DashboardLayout>
...
export default AdminMaintenancesPage;
